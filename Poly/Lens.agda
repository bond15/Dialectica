...
-- TODO: don't be lazy, use the universe levels
module Lens where
...
eq-morphisms : {S T A B : Set} → (v v' : S → A )(u u' : S →  B → T) → (v ⇒ₚ u) ≡ ((v' ⇒ₚ u'))
eq-morphisms = {!   !}
open Category
Bimorphic : {o h : Level} → Category o h
Bimorphic .Ob = Set × Set
Bimorphic ._⇒_ = λ { (S , T) → λ { (A , B) → Lens S T A B} }
Bimorphic ._∘_ = λ  p q → q ⇒∘ₚ p
Bimorphic .id = (λ z → z) ⇒ₚ (λ i z → z )
Bimorphic .idl {x} {y} {f} = {!  eq-morphisms ? ? ? ?  !}
Bimorphic .idr = {!   !}
Bimorphic .assoc = {!   !}
